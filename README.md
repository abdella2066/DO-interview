...
A REST API for feature flags: create flags, turn them on or off globally or for individual users, and evaluate them per user, with a cache in front of Postgres.
...
All `/api/v1` endpoints need an `X-API-Key` header. The health probes are public.
...
app/
  ...
  api/             routes: health, flags, overrides, evaluate
